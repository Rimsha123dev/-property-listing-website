...
const PropertyDetailPage = () => {
  ...
  const [imgSrc, setImgSrc] = useState<string>("/placeholder.jpg"); // default placeholder

  // Decode JWT and set current user id
  useEffect(() => {
    const token = localStorage.getItem("token");
    if (token) {
      try {
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      try {
        ...
        // Update image src when property loads
        if (res.data.image) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-12">
      <div className="grid md:grid-cols-2 gap-10">
        {/* Property Image */}
        <div className="relative h-[400px] w-full rounded-xl shadow-md overflow-hidden">
          <Image
            src={imgSrc}
            alt={property.title}
            fill
        ...
        <div className="space-y-6">
          ...
          <div className="mt-6 border-t pt-4">
            ...
            <div className="flex items-center space-x-4">
              <div className="flex-shrink-0">
                {/* Uncomment and convert to next/image if you want owner's image */}
                {/* <Image
                  src={property.owner?.image || "/user-placeholder.jpg"}
                  alt={property.owner?.name}
                  width={48}
                  height={48}
